...
socket.on('connect', function() {
  ...
  socket.emit('room', `${process.env.ADMIN_TOKEN}-${process.env.COMPANY_ID}-${process.env.CHATBOT_ID}-${process.env.USER_ID}`);
});
...
function ConsoleReporter() {
  ...
  this.specDone = function(result) {
    ...
    if (result.status == 'passed') {
      executableSpecCount++;
      print(colored('green', `Success : ${result.fullName}\n`));
      socket.emit('logs', {
        completed: true,
        result: 0,
        logs: null,
    ...
    if (result.status == 'failed') {
      ...
      executableSpecCount++;
      print(colored('red', `Failed : ${result.fullName}\n`));
      exec(`node /home/botium-bindings/samples/botframework/sendmail.js ${result.failedExpectations[0].message}`, (err, stdout, stderr) => {
      });
      socket.emit('logs', {
        completed: true,
        result: 1,
        logs: result.failedExpectations[0].message.match(/[^\r\n]+/g)[0],
      });
    }
