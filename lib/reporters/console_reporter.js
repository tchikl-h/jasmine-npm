...
function ConsoleReporter() {
  ...
  this.specDone = function(result) {
    ...
    if (result.status == 'failed') {
      ...
      var users = process.env.USERS_EMAIL.split(";");
      users.forEach(user => {
        var mailOptions = {
            from: 'herve.tchikladze@gmail.com',
            to: user.mail,
            subject: `[Test-my-chatbot] Error on chatbot ${process.env.PROJECTNAME}`,
            text: `${result.failedExpectations[0].message}\n${process.env.HOST}/chatbot/${process.env.CHATBOT_ID}`
        };
